...
33. Avoid Premature Optimisation - This is a very common issue, especially when it comes to developers who are just reaching the higher levels of programming. Many mid-senior level programmers find that as they start to understand optimisations they find themselves wanting to write absolutely optimal code. This is never going to be the case, you cannot optimise something that doesn't need it. Premature optimisation is the root of all evil, do not optimise until you need to.

34. Use the module pattern - Avoid pseudoclassical inheritance, JS is not a classical language, using the language as originally intended can lead to bugs and complexity due to the lack of type checking. The module pattern gives us a form of privatisation, lets us modularise our functions, and basically containerises sections of our application. It is also easily transformed into a powerful constructor pattern.

35. Adopt a functional paradigm - JS is a functional language, functional programming in JS I find to be far more enjoyable than using it's pseudo classes, or the horrible prototype syntax. We want to simplify our code, make it smaller, functional programming allows us to do so using the unix methodology of writing small functions which can be put together to perform complex behaviours.